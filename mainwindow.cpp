...
void MainWindow::create_chart(/*final_name, */std::vector<QFrame*> frameArray){
    ...
    for (int i = 0; i < frameArray.size(); i++) {
            ...
            for (int j = 0; j < myNamespace2::N; j++) {
                series->append(j + 0.5, myNamespace2::array[j]);
            }
            ...
            // Get the layout for the current frame
            QVBoxLayout *layout_bigOneChart2 = new QVBoxLayout(frameArray[i]);
...
void MainWindow::on_addTabNameButton_clicked()
{
    ...
    for(int i = 0; i < parses; i++) {
        ...
        if(i == 0){
            ...
            frameArray[i]->setStyleSheet("QFrame { background-color: black; }");
...
void MainWindow::chartCreationSetup(int parses, std::vector<std::string> preferences_split, std::vector<QFrame*> frameArray){
    for(int i=2; i<=parses+1; i++){
        Firestore_Read_Data(preferences_split.at(i).c_str());
        create_chart(frameArray);
    }
